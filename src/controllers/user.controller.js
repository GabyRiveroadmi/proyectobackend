...
class UserController {
    async register(req, res) {
        ...
        try {
            ...
            }, "MiProyecto1", { expiresIn: "1h" });

            res.cookie("jwtToken", token, {maxAge: 3600000, httpOnly: true});
            
            res.redirect("/api/sessions/current");
        } catch (error) {
            res.status(500).send("Error del server para el registro de usuario");
            (console.error(error))
        }
    }
    ...
    async login(req, res) {
        ...
        try {
            ...
            const token = jwt.sign({
                ...
                email: user.email,
                role: user.role
            }, "MiProyecto1", {expiresIn: "24h"});

            res.cookie("jwtToken", token, {maxAge: 3600000, httpOnly: true});
            res.redirect("/api/sessions/current");
        } catch (error) {
            res.status(500).send("Error del server en el login");
        }
    }
